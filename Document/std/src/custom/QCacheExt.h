...
class QCacheExt
{
    ...
    qsizetype mx = 0;
    qsizetype total = 0;
    std::function<void(const Key&)> _onEraseFn;
    ...
    void unlink(Node *n) noexcept(std::is_nothrow_destructible_v<Node>)
    {
        ...
        if (_onEraseFn) _onEraseFn(n->key); // customization
        d.erase(it);
    }
    ...
    Q_DISABLE_COPY(QCacheExt)

public:
    inline explicit QCacheExt(qsizetype maxCost = 100, std::function<void(const Key&)> onEraseFn = nullptr) noexcept
        : mx(maxCost)
        , _onEraseFn(onEraseFn)
    {
    }
    inline ~QCacheExt()
    {
    ...
    void setOnEraseFn(std::function<void(const Key&)> onEraseFn)
    {
        _onEraseFn = onEraseFn;
    }
